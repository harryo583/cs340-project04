# This version is theoretically more optimal as it avoids duplicate calls of the function
...
def scan(url):
    ...
    # Compute scan_time
    try:
        result['scan_time'] = scan_time(url)
    except Exception as e:
        result['scan_time'] = f"Error: {str(e)}"

    # Compute ipv4_addresses
    try:
        result['ipv4_addresses'] = ipv4_addresses(url)
        ipv4s = result['ipv4_addresses']
    except Exception as e:
        result['ipv4_addresses'] = f"Error: {str(e)}"
        ipv4s = []

    # Compute ipv6_addresses
    try:
        result['ipv6_addresses'] = ipv6_addresses(url)
    except Exception as e:
        result['ipv6_addresses'] = f"Error: {str(e)}"

    # Compute http_server
    try:
        result['http_server'] = http_server(url)
    except Exception as e:
        result['http_server'] = f"Error: {str(e)}"

    # Compute insecure_http
    try:
        result['insecure_http'] = insecure_http(url)
    except Exception as e:
        result['insecure_http'] = f"Error: {str(e)}"

    # Compute redirect_to_https
    try:
        result['redirect_to_https'] = redirect_to_https(url)
    except Exception as e:
        result['redirect_to_https'] = f"Error: {str(e)}"

    # Compute hsts
    try:
        result['hsts'] = hsts(url)
    except Exception as e:
        result['hsts'] = f"Error: {str(e)}"

    # Compute tls_versions
    try:
        result['tls_versions'] = tls_versions(url)
    except Exception as e:
        result['tls_versions'] = f"Error: {str(e)}"

    # Compute root_ca
    try:
        result['root_ca'] = root_ca(url)
    except Exception as e:
        result['root_ca'] = f"Error: {str(e)}"

    # Now, functions that need ipv4s
    # rdns_names(url, ipv4s)
    try:
        result['rdns_names'] = rdns_names(url, ipv4s)
    except Exception as e:
        result['rdns_names'] = f"Error: {str(e)}"

    # rtt_range(url, ipv4s)
    try:
        result['rtt_range'] = rtt_range(url, ipv4s)
    except Exception as e:
        result['rtt_range'] = f"Error: {str(e)}"

    # geo_locations(url, ipv4s)
    try:
        result['geo_locations'] = geo_locations(url, ipv4s)
    except Exception as e:
        result['geo_locations'] = f"Error: {str(e)}"
